...
pub fn calorie_counter() -> u32 {
    ...
    for word in words {
        if word == "" {
            let smallest_current_highest_calories =
                smallest_array_element(three_current_highest_calories);

            if current_calories > smallest_current_highest_calories.1 {
                three_current_highest_calories[smallest_current_highest_calories.0] =
                    current_calories;
            }
            current_calories = 0;
        } else {
